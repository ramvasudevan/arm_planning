...
P3. handle the output, release the memory
	*/
	...
	plhs[3] = mxCreateNumericMatrix(links.n_links * links.n_time_steps * links.Z_width, (reduce_order * 2 - 1), mxDOUBLE_CLASS, mxREAL);
	double *output4 = mxGetPr(plhs[3]);
	for (uint32_t i = 0; i < links.n_links * links.n_time_steps; i++) {
		for (uint32_t j = 0; j < (reduce_order * 2 - 1); j++) {
			for (uint32_t k = 0; k < links.Z_width; k++) {
				output4[(j * links.n_links * links.n_time_steps + i) * links.Z_width + k] = links.RZ_new[(i * (reduce_order * 2 - 1) + j) * links.Z_width + k];
			}
		}
	}
