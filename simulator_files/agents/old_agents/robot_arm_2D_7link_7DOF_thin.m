classdef robot_arm_2D_7link_7DOF_thin < robot_arm_agent
    methods
        function A = robot_arm_2D_7link_7DOF_thin(varargin)
            n_links_and_joints = 7 ;
            
            dimension = 2 ;
            
            link_shapes = {'oval','oval','oval','oval','oval','oval','oval'} ;
            
            link_sizes = [1, 1, 1, 1, 1, 1, 1 ;
                          0.001, 0.001, 0.001, 0.001, 0.001, 0.001, 0.001] ;

            joint_locations = [0.000 0.5 0.5 0.5 0.5 0.5 0.5  ;
                               0.001 0 0 0 0 0 0 ;
...
%                                   +Inf, +Inf, +Inf] ;
            joint_state_limits = ones(2,7).*[-inf;inf];
            
%             joint_speed_limits = [-pi/2, -pi/2, -pi/2;
%                                   +pi/2, +pi/2, +pi/2] ;
            joint_speed_limits = ones(2,7).*[-pi/2;pi/2];

%             joint_input_limits = 10.*[-1, -1, -1 ;
%                                       +1, +1, +1 ] ;

            joint_input_limits = ones(2,7).*[-10;10];
