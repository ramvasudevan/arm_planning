function [X] = generate_x_plan_from_k(R_0, phi_dot_0, k, options)
...
% 
phi_to_peak = 0 + phi_dot_0*options.t_plan + 1/2*phi_ddot_to_peak*options.t_plan^2;
% phi_to_stop = phi_to_peak(:, end) + k1.*(t_to_stop - options.t_plan) + 1/2*phi_ddot_to_stop.*(t_to_stop - options.t_plan).^2;
theta_to_peak = 0 + theta_dot_0*options.t_plan + 1/2*theta_ddot_to_peak*options.t_plan^2;
...
%     xout{i} = make_orientation(phi_to_peak(i), 3)*make_orientation(theta_to_peak(i), 2)*[options.L; 0; 0];
end
...
%         R{j} = R_0{j}*make_orientation(theta(j, i), 2)*make_orientation(phi(j, i), 3); % ?? theta cuz i switched around in dynamics
            xout{j} = make_orientation(phi_to_peak(j), 3)*make_orientation(-theta_to_peak(j), 2)*[options.L; 0; 0];
        if j == 1
%             X(3*(j-1)+1:3*j, i) = R{j}*[options.L; 0; 0];
            X(3*(j-1)+1:3*j, :) = R_0{j}*xout{j};
        else
%             X(3*(j-1)+1:3*j, :) = R{j}*[options.L; 0; 0] + X(3*(j-2)+1:3*(j-1), i);
            X(3*(j-1)+1:3*j, :) = R_0{j}*xout{j} + X(3*(j-2)+1:3*(j-1), :);
        end
    end
