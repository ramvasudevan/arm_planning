classdef robot_arm_dynamics_rotatotope_RTD_planner_3D_fetch < robot_arm_generic_planner
    %% properties
    properties
        trajopt_start_tic
    ...
    methods
        %% constructor
        function P = robot_arm_dynamics_rotatotope_RTD_planner_3D_fetch(varargin)
            t_move = 0.5;
...
%             q_dot_0 = agent_info.state(P.arm_joint_speed_indices, end) ;
            ...
            % get actual initial conditions of the robot
            P.q_act_0 = agent_info.state(P.arm_joint_state_indices, end) ;
            P.q_dot_act_0 = agent_info.state(P.arm_joint_speed_indices, end) ;
            
            % generate error
            e_0 = zeros(6,1);
            e_dot_0 = zeros(6,1);
            
%             e_0 = 2 * pi/24 .* rand(6,1) - pi/24;
%             e_dot_0 = 2 * (0.2670/2) .* rand(6,1) - (0.2670/2);
%             for e_idx = 1:6
%                 p = rand(1,1);
%                 if p < 0.35
%                     e_0(e_idx) = 2 * pi/12 * rand(1,1) - pi/12;
%                 end
% 
%                 p = rand(1,1);
%                 if p < 0.35
%                     e_dot_0(e_idx) = 2 * 0.2670 * rand(1,1) - 0.2670;
%                 end
%             end
            
            % add error to create desired initial condition
            
            P.error = [e_0; e_dot_0];
            
            q_0 = P.q_act_0 - e_0;
            q_dot_0 = P.q_dot_act_0 - e_dot_0; 
            
            
            P.q_des_0 = q_0;
            P.q_dot_des_0 = q_dot_0;
            
            if ~P.use_cuda_flag
                % generate FRS
                
                % create rotatotope
                P.R = robot_arm_dynamics_FRS_rotatotope_fetch(q_0, q_dot_0, e_0, e_dot_0, P.FRS_options);
...
%             end
        ...
        function [T, U, Z] = generate_trajectory(P, q_0, q_dot_0, k_opt)
            ...
            % generate to peak error            
            initial_error = P.error;
            to_peak_err_fun = expm([0*eye(6) eye(6); -P.Kp -P.Kd]*t)*initial_error;
            

            to_peak_err = [];
            
            for time_step = 1:length(T_plan)
                to_peak_err = [to_peak_err subs(to_peak_err_fun, T_plan(time_step))];
            end
            
            q_to_peak_err = to_peak_err(1:6,:);
            q_dot_to_peak_err = to_peak_err(7:12,:);
    
            t_to_stop = T_brake(end) - T_brake(1);
            U = zeros(P.arm_n_inputs,length(T));
            
            q_to_peak = q_0 + q_dot_0.*T_plan + (1/2)*k_opt.*T_plan.^2 + q_to_peak_err;
            q_dot_to_peak = q_dot_0 + k_opt.*T_plan + q_dot_to_peak_err;
            q_dot_dot_to_peak = k_opt;
            
            q_peak = q_to_peak(:, end);
            q_dot_peak = q_dot_to_peak(:, end);
            q_dot_dot_peak = repmat(q_dot_dot_to_peak(:,1), 1, length(T_plan));
            
            %~~~~~~~~~~ STOPPING
            T_brake = T_brake - T_brake(1);
            
            % generate to stop error
            error_at_peak = to_peak_err(:,end);    
            to_stop_err_fun = expm([0*eye(6) eye(6); -P.Kp -P.Kd]*t)*error_at_peak;
            
            to_stop_err = [];

            for time_step = 1:length(T_brake)
                to_stop_err = [to_stop_err subs(to_stop_err_fun, T_brake(time_step))];
            end
            
            q_to_stop_err = to_stop_err(1:6,:);
            q_dot_to_stop_err = to_stop_err(7:12,:);            
   
            q_to_stop = q_peak + q_dot_peak.*T_brake + (1/2)*((0 - q_dot_peak)./t_to_stop).*T_brake.^2 + q_to_stop_err;
            q_dot_to_stop = q_dot_peak + ((0 - q_dot_peak)./t_to_stop).*T_brake + q_dot_to_stop_err;
            ...
            for i = 1:size(q_to_peak, 1)
               Z(2*i-1:2*i, :) =  [q_to_peak(i, :), q_to_stop(i, :); q_dot_to_peak(i, :), q_dot_to_stop(i, :)];% q_dot_dot_peak(i,:), q_dot_dot_to_stop(i,:)];
            end
        end
        
%         function [err_to_peak, err_to_stop] = generate_error_trajectory(e_0, e_dot_0)
%         
%         
%         end
