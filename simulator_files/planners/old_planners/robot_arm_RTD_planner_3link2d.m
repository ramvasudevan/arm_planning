classdef robot_arm_RTD_planner_3link2d < robot_arm_generic_planner
    properties
    ...
    methods
        %% constructor
        function P = robot_arm_RTD_planner_3link2d(varargin)
            t_move = 0.5;
...
%             O = O(2:end);
            ...
            [links, k_lim, k_unsafe_A, k_unsafe_b] = compute_unsafe_parameters_3link2d(q_0, q_dot_0, O);
...
%                 Z = nan(P.arm_n_states,N_T) ;
                if ~isnan(P.q_0_prev) % if prev trajectory was not a braking traj
                    ...
                    Z = ones(6, length(T)).*[q_0(1); 0; q_0(2); 0; q_0(3); 0];
                    P.q_0_prev = nan;
                    P.q_dot_0_prev = nan;
                    P.k_opt_prev = nan;
                end
...
%             end
        ...
        function [q_plan] = compute_q_plan(P, q_0, q_dot_0, q_dot_pk)
            ...
            q_plan(3,1) = q_0(3) + q_dot_0(3)*P.t_plan + ((q_dot_pk(3) - q_dot_pk(2) - q_dot_0(3))/2)*P.t_plan;
        end
...
%             cost = 0.75*(q_plan(1)-q_des(1))^2 + 0.25*(q_plan(2)-q_des(2))^2;
        ...
        function [k_opt] = trajopt(P, k_lim, k_unsafe_A, k_unsafe_b, q_0, q_dot_0, q_des)
            ...
            lb = [min(k_lim{2}.V(1, :)); min(k_lim{2}.V(2, :)); min(k_lim{3}.V(3, :))];
            ub = [max(k_lim{2}.V(1, :)); max(k_lim{2}.V(2, :)); max(k_lim{3}.V(3, :))];
